...
The EventProcessor exists to generalize the allocation logic and make it abstract.

The event processor allocation logic varies based on the expected behavior. We support two modes:
...
Depending on the required behavior, we use different event processor allocators at different levels.

For instance, to support synchronous processing, we allocate a worker (event processor at the trigger level).
While the allocated event processor is waiting for a response, it cannot be allocated again. To enforce this, we use a sync pool allocator,
which ensures that each event processor is assigned to only one event at a time.
