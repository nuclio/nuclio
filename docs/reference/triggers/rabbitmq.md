...
| topics            | list of strings    | If specified, the trigger declares the exchange and queue (using the configured `exchangeName` and `queueName`), then binds the queue to the exchange using these topics as routing keys. |
