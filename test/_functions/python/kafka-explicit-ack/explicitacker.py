...
def get_last_commit_offset():
    with open(last_committed_offset_file_path, "r") as file:
        ...
        return events[0] if len(events) > 0 else 0
...
async def handler(context, event):
    ...
    context.logger.info("TOMER - Received event: {0}, event.trigger.kind: {1}".format(event, event.trigger.kind))
    ...
    if event.trigger.kind == "http":
        ...
        # body = await handle_event(context, event)
        body = event.body.decode('utf-8').lower()

        if "queue size" in body:
            queue_size = get_number_of_enqueued_events()
            context.logger.info("current queue size: {0}".format(queue_size))
            response_body = "current queue size: {0}".format(queue_size)

        elif "commit offset" in body:
            last_commit_offset = get_last_commit_offset()
            context.logger.info("last committed offset: {0}".format(last_commit_offset))
            response_body = "last committed offset: {0}".format(last_commit_offset)

        elif "start processing" in body:
            context.logger.info("start processing")
            await process_events(context)
            response_body = "started processing"
