...
func NewFunctionMonitor(parentLogger logger.Logger,
	...
	kubeClientSet kubernetes.Interface,
	nuclioClientSet nuclioioclient.Interface,
	pollingInterval time.Duration) (*FunctionMonitor, error) {
	...
	go func() {
		defer func() {
			if err := recover(); err != nil {
				...
				fm.logger.ErrorWith("Panic caught while starting function monitoring",
					"err", err,
		...
		for {
			select {
			case <-time.After(fm.pollingInterval):
...
func (fm *FunctionMonitor) checkFunctionStatuses() error {
	...
	for _, function := range functions.Items {
		...
		errGroup.Go(func() error {
			return fm.checkFunctionStatus(&function)
		})
	}
...
func (fm *FunctionMonitor) checkFunctionStatus(function *nuclioio.NuclioFunction) error {
	...
	if functionIsAvailable && function.Status.State == functionconfig.FunctionStateError {
		fm.logger.InfoWith("Function has recovered, removing error and set state as ready",
			"functionName", function.Name,
			"functionNamespace", function.Namespace,
			"functionStatusMessage", function.Status.Message)
