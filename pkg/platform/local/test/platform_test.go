...
func (suite *TestSuite) TestRunFunctionContainerWithCustomRestartPolicy() {
	...
	createFunctionOptions := suite.GetDeployOptions("restartable", filepath.Join(functionPath...))
...
func (suite *TestSuite) getDeployOptions(functionName string) *platform.CreateFunctionOptions {
	...
	createFunctionOptions := suite.GetDeployOptions(functionName, filepath.Join(functionPath...))
