...
import (
	...
	"github.com/nuclio/nuclio/pkg/nuctl2/deployer"
...
func newDeployCommandeer(rootCommandeer *RootCommandeer) *deployCommandeer {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			functionDeployer, err := deployer.NewFunctionDeployer(logger, platformInstance)
			if err != nil {
				return errors.Wrap(err, "Failed to create function runner")
			}

			_, err = functionDeployer.Deploy(&commandeer.deployOptions)
			return err
		},
	}

	addRunFlags(cmd, &commandeer.deployOptions, &commandeer.encodedDataBindings)
...
func addRunFlags(cmd *cobra.Command, options *platform.DeployOptions, encodedDataBindings *string) {
