...
func (rc *RootCommandeer) initialize() (platform.Platform, nuclio.Logger, error) {
	logger, err := rc.createLogger()
	if err != nil {
		return nil, nil, errors.Wrap(err, "Failed to create logger")
	}

	platformInstance, err := rc.createPlatform(logger)
	if err != nil {
		return nil, nil, errors.Wrap(err, "Failed to create logger")
	}

	return platformInstance, logger, nil
}
