...
func newInvokeCommandeer(rootCommandeer *RootCommandeer) *invokeCommandeer {
	...
	cmd.Flags().StringVarP(&commandeer.invokeVia, "via", "", "any", "Invoke the function via - \"any\" (default): a load balancer or an external IP; \"loadbalancer\": a load balancer; \"external-ip\": an external IP")
