...
func (suite *projectTestSuite) TestDeleteWithFunctions() {
	for _, testCase := range []struct {
	...
	}{
		{
			...
			requestHeaders: map[string]string{
				"x-nuclio-delete-project-strategy": string(platform.DeleteProjectStrategyCascade),
			},
			expectedStatusCode: http.StatusNoContent,
		},
		{
			...
			requestHeaders: map[string]string{
				"x-nuclio-delete-project-strategy": string(platform.DeleteProjectStrategyRestrict),
			},
		...
		{
			name: "DefaultDeleteProjectStrategyToRestrict",
			deleteProjectOptions: &platform.DeleteProjectOptions{
				Meta: platform.ProjectMeta{
					Name:      "p1",
					Namespace: "p1-namespace",
