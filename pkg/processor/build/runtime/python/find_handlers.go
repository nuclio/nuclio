...
# If this file ever changes, run:
#   ./find_handlers.py --embed-in-go
...
def embed_in_go():
    py_file = abspath(__file__)
    go_file = py_file.replace('.py', '.go')
    header = dedent('''\
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'path', help='path to discover', type=FileType(), nargs='?',
        default='-')
    parser.add_argument(
        '--embed-in-go', action='store_true', help='embed in go file',
        default=False)
